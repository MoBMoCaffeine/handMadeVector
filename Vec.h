...
private:
    ...
    void merge_arrays(int left, int mid, int right)
    {
        ...
        while (i < n1) data[k++] = std::move(leftArr[i++]);
        while (j < n2) data[k++] = std::move(rightArr[j++]);
...
public:
    ...
    Vec(Vec<T> &&other) noexcept : data(nullptr), capacity(0), length(0) { swap(other); }
    ...
    void pop_back()
    {
        if (length == 0) throw std::out_of_range("pop_back() called on empty Vec");
        length--;
    }
    ...
    T &front()
    {
        if (length == 0) throw std::out_of_range("front() called on empty Vec");
        return data[0];
    }

    T &back()
    {
        if (length == 0) throw std::out_of_range("back() called on empty Vec");
        return data[length - 1];
    }

    void insert(size_t index, const T &value)
    {
        if (index > length) throw std::out_of_range("insert() index out of range");
        if (length == capacity)
        {
            size_t new_capacity = (capacity == 0) ? 1 : capacity * 2;
            resize_capacity(new_capacity);
    ...
    {
        if (index > length) throw std::out_of_range("emplace() index out of range");
        if (length == capacity)
        {
            size_t new_capacity = (capacity == 0) ? 1 : capacity * 2;
            resize_capacity(new_capacity);
    ...
    void sort()
    {
        if (length > 1) merge_sort(0, length - 1);
    }
    ...
    {
        if (n > capacity) resize_capacity(n);
    ...
    {
        if (ilist.size() > capacity) resize_capacity(ilist.size());
        size_t i = 0;
    ...
    void assign(InputIt first, InputIt last)
    {
        ...
        if (new_size > capacity) resize_capacity(new_size);
        size_t i = 0;
    ...
    {
        if (index >= length) throw std::out_of_range("erase() index out of range");
    ...
    {
        if (first >= length || last > length || first >= last)
            throw std::out_of_range("erase() range invalid");
        size_t range_size = last - first;
        for (size_t i = first; i + range_size < length; ++i)
    ...
    bool operator>(const Vec<T> &other) const { return other < *this; }
    bool operator<=(const Vec<T> &other) const { return !(other < *this); }
    bool operator>=(const Vec<T> &other) const { return !(*this < other); }
    ...
    friend std::ostream &operator<<(std::ostream &os, const Vec<T> &vec)
    {
        ...
        {
            os << vec.data[i];
            if (i + 1 < vec.length) os << ", ";
        }
        os << "]";
        return os;
    }

    friend std::istream &operator>>(std::istream &is, Vec<T> &vec)
    {
        size_t n;
        is >> n;
        vec.resize(n);
